...
use crate::lz_decode::LZDecoder;
use crate::segment_decompression::decompress_segment_with_marker;
...
impl Decompressor {
    ...
    fn get_segment(&mut self, desc: &SegmentDesc) -> Result<Contig> {
        ...
        if desc.group_id >= NO_RAW_GROUPS {
            ...
            let mut lz_decoder = LZDecoder::new(self.min_match_len);
            lz_decoder.prepare(reference);

            // Handle empty encoding (segment equals reference)
            let decoded = if lz_encoded.is_empty() {
                reference.clone()
            } else {
                lz_decoder.decode(&lz_encoded)
            };
    ...
    ///
    /// let mut dec = Decompressor::open("data.agc", DecompressorConfig::default())?;
    /// dec.write_sample_fasta("sample1", Path::new("output.fasta"))?;
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    pub fn write_sample_fasta(&mut self, sample_name: &str, output_path: &std::path::Path) -> Result<()> {
        ...
        for (contig_name, contig_data) in contigs {
            ...
            // and write in 80-character lines
            let mut ascii_line = Vec::with_capacity(80);
            for &base in &contig_data {
                ...
                ascii_line.push(ascii_base);
                if ascii_line.len() == 80 {
                    writer.write_all(&ascii_line)?;
                    writeln!(writer)?;
                    ascii_line.clear();
                }
            }
            if !ascii_line.is_empty() {
                writer.write_all(&ascii_line)?;
                writeln!(writer)?;
            }
        }
...
// Note: Integration tests that require creating archives are in ragc-writer or ragc-cli
