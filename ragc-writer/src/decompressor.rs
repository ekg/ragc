...
/// # Example
/// ```no_run
/// use ragc_writer::{Decompressor, DecompressorConfig};
/// use std::thread;
///
...
mod tests {
    ...
    fn test_decompressor_basic() {
        ...
        // Decompress
        {
            let config = DecompressorConfig { verbosity: 2, ..Default::default() }; // Enable debug output
    ...
    fn test_decompressor_multiple_contigs() {
        ...
        // Decompress
        {
            let config = DecompressorConfig { verbosity: 0, ..Default::default() };
            let mut decompressor = Decompressor::open(archive_path, config).unwrap();
