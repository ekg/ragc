...
use ragc_core::{Archive, CollectionV3, Contig, CONTIG_SEPARATOR};
...
/// # Example
/// ```no_run
/// use ragc_writer::{StreamingQueueCompressor, StreamingQueueConfig};
        ...
        for (place, segment) in segments.iter().enumerate() {
            ...
            // to avoid moving segment_data prematurely
            {
                ...
                let buffer = groups.entry(key.clone()).or_insert_with(|| {
                    ...
                    let archive_version =
                        ragc_core::AGC_FILE_MAJOR * 1000 + ragc_core::AGC_FILE_MINOR;
                    let delta_stream_name =
                        ragc_core::stream_delta_name(archive_version, group_id);
                    let ref_stream_name = ragc_core::stream_ref_name(archive_version, group_id);
