...
pub struct SharedCompressorState {
    ...
    pub collection: Option<Arc<Mutex<ragc_core::CollectionV3>>>,
    ...
    if no_new > 0 {
        ...
        if let Some(archive_mutex) = &shared.archive {
            ...
            for i in 0..no_new {
                ...
                archive.register_stream(&ragc_core::stream_delta_name(3000, seg_num));
                archive.register_stream(&ragc_core::stream_ref_name(3000, seg_num));
            }
        }
                    ...
                    && v_segments[group_id as usize].is_none()
                {
                    ...
                    if let Some(archive_mutex) = &shared.archive {
                        ...
                        let stream_id = archive
                            .get_stream_id(&ragc_core::stream_delta_name(3000, group_id as u32))
                            .unwrap_or(0);
                        let ref_stream_id = archive
                            .get_stream_id(&ragc_core::stream_ref_name(3000, group_id as u32))
                            .unwrap_or(0);
                        drop(archive);
...
pub fn create_agc_archive(
    ...
    // Write file_type_info stream (C++ AGC compatibility)
    {
        ...
        append_str(&mut data, &ragc_core::AGC_FILE_MAJOR.to_string());

        append_str(&mut data, "producer_version_minor");
        append_str(&mut data, &ragc_core::AGC_FILE_MINOR.to_string());
        ...
        append_str(
            ...
            &format!(
                "RAGC v.{}.{}",
                ragc_core::AGC_FILE_MAJOR,
                ragc_core::AGC_FILE_MINOR
            ),
        );
...
fn compress_samples_streaming_with_archive(
    ...
    collection: Option<Arc<Mutex<ragc_core::CollectionV3>>>,
) -> Result<(), String> {
