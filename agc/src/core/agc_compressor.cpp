        ...
        for (int group_id = block_group_id; group_id > block_group_id - CBufferedSegPart::part_id_step; --group_id)
        {
            if (!buffered_seg_part.is_empty_part(group_id))
                while (buffered_seg_part.get_part(group_id, kmer1, kmer2, sample_name, contig_name, seg_data, is_rev_comp, seg_part_no))
                {
                    ...
                    // TRACE: Log segment addition for groups 21-30
                    if (group_id >= 21 && group_id <= 30) {
                        cerr << "[CPP] SEG_ADD: gid=" << group_id
                             << " sample=" << sample_name << " contig=" << contig_name
                             << " part=" << seg_part_no << " len=" << seg_data.size()
                             << " kmers=(" << hex << kmer1 << "," << kmer2 << dec << ")" << endl;
                    }
                }
        }
        ...
        else
        {
            ...
            if (split_match.first != ~0ull)
            {
                ...
                else
                {
                    ...
                    // TRACE: Log split for groups 21-30
                    if (segment_id >= 21 && segment_id <= 30 || segment_id2 >= 21 && segment_id2 <= 30) {
                        cerr << "[CPP] SPLIT_ACCEPT: from_kmers=(" << hex << kmer_front.data() << "," << kmer_back.data()
                             << ") to_left=(" << pk.first << "," << pk.second
                             << ") to_right=(" << pk2.first << "," << pk2.second
                             << dec << ") pos=" << seg2_start_pos << " mid=" << hex << split_match.first << dec << endl;
                    }
                }
            }
        }
